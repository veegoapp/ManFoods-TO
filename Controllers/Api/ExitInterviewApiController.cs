...
public class ExitInterviewApiController : ControllerBase
{
    ...
    private static ExitInterviewFilter BuildFilter(string? store, string? storeLeader, string? oc, string? om) =>
        new() { Store = store, StoreLeader = storeLeader, OperationConsultant = oc, OperationManager = om };
    ...
    {
        var (role, assignedName) = Identity();
        return Ok(await _exitInterviews.GetReasonsForLeavingAsync(BuildFilter(store, storeLeader, oc, om), role, assignedName));
    }
    ...
    {
        var (role, assignedName) = Identity();
        return Ok(await _exitInterviews.GetWouldReturnAsync(BuildFilter(store, storeLeader, oc, om), role, assignedName));
    }
    ...
    {
        var (role, assignedName) = Identity();
        return Ok(await _exitInterviews.GetOverallExperienceAsync(BuildFilter(store, storeLeader, oc, om), role, assignedName));
    }
    ...
    {
        var (role, assignedName) = Identity();
        return Ok(await _exitInterviews.GetWorkloadConditionAsync(BuildFilter(store, storeLeader, oc, om), role, assignedName));
    }
    ...
    {
        var (role, assignedName) = Identity();
        return Ok(await _exitInterviews.GetEngagementDriversAsync(BuildFilter(store, storeLeader, oc, om), role, assignedName));
    }
    ...
    {
        var (role, assignedName) = Identity();
        return Ok(await _exitInterviews.GetCommentsAsync(BuildFilter(store, storeLeader, oc, om), role, assignedName));
    }
}
